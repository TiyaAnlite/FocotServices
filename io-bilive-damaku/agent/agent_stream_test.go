...
func BindEvent(mq *nats.Conn, prefix, agentId string) {
	...
	_, _ = mq.Subscribe(prefix+".agent.fansMedal", AutoResponseEventShow(&agent.FansMedalMeta{}))
	_, _ = mq.Subscribe(prefix+".agent.userInfoMeta", AutoResponseEventShow(&agent.UserInfoMeta{}))
	_, _ = mq.Subscribe(prefix+".agent.damaku", EventShow(&agent.Damaku{}))
	_, _ = mq.Subscribe(prefix+".agent.gift", EventShow(&agent.Gift{}))
	_, _ = mq.Subscribe(prefix+".agent.guard", EventShow(&agent.Guard{}))
	_, _ = mq.Subscribe(prefix+".agent.superChat", EventShow(&agent.SuperChat{}))
	_, _ = mq.Subscribe(prefix+".agent.online", EventShow(&agent.OnlineRankCount{}))
	_, _ = mq.Subscribe(prefix+".agent.onlineV2", EventShow(&agent.OnlineRankV2{}))
}
